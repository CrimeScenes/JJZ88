    ...
    if Key:lower() == shared.Forbidden.Target.Keybind:lower() then
        if shared.Forbidden.PointAssist.Enabled then
            if IsTargeting then
                ...
                if newTarget and newTarget.Character and newTarget.Character:FindFirstChildOfClass("Humanoid").Health >= 7 and IsVisible(newTarget.Character) then
                    TargetPlayer = newTarget  
                end
            else
                local initialTarget = ClosestPlrFromMouse() 
                if initialTarget and initialTarget.Character and initialTarget.Character:FindFirstChildOfClass("Humanoid").Health >= 7 and IsVisible(initialTarget.Character) then
                    IsTargeting = true
...
local function IsVisible(targetPlayer)
    if targetPlayer and targetPlayer.Character then
        ...
        -- If there is no hit, the target is visible, else it's blocked by something
            ...
            if shared.Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                ...
                elseif shared.Forbidden.Silent.mode == "normal" then
                    ...
                    if targetToShoot and targetToShoot.Character then
                        if IsVisible(targetToShoot.Character) then
                            local closestPoint
                            if shared.Forbidden.Silent.TargetMode == "OptimalTargetPoint" then
...
RunService.RenderStepped:Connect(function()
    ...
    -- Silent Mode Targeting
    if shared.Forbidden.Silent.Enabled and IsTargeting then  
        UpdateFOV()  

        -- Mode: Targeting Specific Player
        if shared.Forbidden.Silent.mode == "target" and TargetPlayer then
            if TargetPlayer.Character then
                local targetPos = TargetPlayer.Character.Head.Position
                -- Check health and visibility
                if TargetPlayer.Character.Humanoid.Health < 7 or not IsVisible(targetPos) then
                    TargetPlayer = nil
                    IsTargeting = false
                    LastTarget = TargetPlayer  -- Save Last Target if lost
                    return
                end
        ...
        -- Mode: Normal Targeting based on mouse
        elseif shared.Forbidden.Silent.mode == "normal" then
            ...
            if target and target.Character then
                ...
                -- Check health and visibility
                if target.Character.Humanoid.Health < 7 or not IsVisible(targetPos) then
                    return  
                end
    ...
    -- Restoring Last Target if it becomes visible
    elseif LastTarget and LastTarget.Character then
        ...
        if IsVisible(lastTargetPos) then
            ...
            IsTargeting = true
            LastTarget = nil  -- Clear the stored Last Target once it's re-targeted
        end
    else
        -- If nothing is targeted, hide the FOV indicator
        Fov.Visible = false  
    end
end)
